...
# ZSH
sudo apt-get install zsh && zsh --version && chsh -s $(which zsh)
...
# ZSH autosuggestions
mkdir ~/.zsh/zsh-autosuggestions && git clone https://github.com/zsh-users/zsh-autosuggestions ~/.zsh/zsh-autosuggestions
...
# Foreman
gem install foreman && foreman --version

# Elixir
asdf plugin add erlang && asdf install erlang 23.2 && asdf plugin add elixir && asdf install elixir 1.10 && asdf global elixir 1.10
...
# Image magick
sudo apt-get update && sudo apt-get install ghostscript imagemagick && magick --version
